      ...
      ["cancelled", "expired", "suspended", "incomplete"].includes(subscription.status)
        ...
        : (() => {
            return listBillingPlans().then(({ items }) => {
              const oldPlan = items.find(
                (item) => item.code === subscription.plan_code,
              );
              const direction =
                oldPlan !== undefined &&
  ...
  return (
    <section aria-labelledby="checkout-heading">
      <PageHeader
        ...
        description="Continue to a secure hosted payment page. Card data never enters FactoryMind."
      />
      <Link
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
          <form className={billingPanelClassName} onSubmit={submit}>
            ...
            <div
              ...
              role="note"
            >
              After payment, FactoryMind waits for verified confirmation before changing
              your plan. Closing this page does not lose a completed payment.
            </div>
            <button
            ...
            >
              {busy
                ...
                : `Continue to payment · ${formatMoney(plan.monthly_price_minor)}`}
            </button>
          </form>
