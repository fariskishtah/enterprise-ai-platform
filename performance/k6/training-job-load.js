...
export function setup() {
  if (!enabled("ENABLE_TRAINING_LOAD")) {
    fail(
      "Training load is disabled; set ENABLE_TRAINING_LOAD=true explicitly.",
    );
  }
...
export default function trainingJobLoad(data) {
  ...
  for (let poll = 0; poll < maxPolls; poll += 1) {
    ...
    if (["succeeded", "failed", "cancelled"].includes(jobStatus)) {
      check(jobStatus, {
        "training reached succeeded terminal state": (value) =>
          value === "succeeded",
      });
      return;
    }
...
export function teardown(data) {
  logout(data.refreshSession, "training_auth_logout");
}
