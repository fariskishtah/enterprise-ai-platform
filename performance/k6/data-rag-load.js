...
export function setup() {
  return credentialsConfigured()
    ...
    : { accessToken: null, refreshSession: null };
}
...
export default function dataRagLoad(data) {
  ...
  const datasetsRes = http.get(`${BASE_URL}/ai/datasets?limit=20&offset=0`, {
    ...bearerParams(data.accessToken, "datasets_list"),
    tags: { type: "read" },
  });
  check(datasetsRes, {
  ...
  if (Array.isArray(datasets) && datasets.length > 0) {
    ...
    const versionsRes = http.get(
      ...
      {
        ...bearerParams(data.accessToken, "dataset_versions"),
        tags: { type: "read" },
      },
    );
    check(versionsRes, {
  ...
  const jobsRes = http.get(`${BASE_URL}/ai/training-jobs?limit=20&offset=0`, {
    ...bearerParams(data.accessToken, "training_jobs_list"),
    tags: { type: "read" },
  });
  check(jobsRes, {
  ...
  const automlRes = http.get(
    ...
    {
      ...bearerParams(data.accessToken, "automl_studies_list"),
      tags: { type: "read" },
    },
  );
  check(automlRes, {
  ...
  const kbRes = http.get(
    ...
    {
      ...bearerParams(data.accessToken, "knowledge_bases_list"),
      tags: { type: "read" },
    },
  );
  check(kbRes, {
  ...
  if (Array.isArray(kbs) && kbs.length > 0) {
    ...
    const searchRes = http.post(
      ...
      {
        ...jsonParams("knowledge_base_search", data.accessToken),
        tags: { type: "read" },
      },
    );
    check(searchRes, {
  ...
  const convsRes = http.get(
    ...
    {
      ...bearerParams(data.accessToken, "chat_conversations_list"),
      tags: { type: "read" },
    },
  );
  check(convsRes, {
...
export function teardown(data) {
  logout(data.refreshSession, "rag_auth_logout");
}
